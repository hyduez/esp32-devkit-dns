...
const char *blockedDomains[] = {
    ...
    "al-array.com",
    "tracking.al-array.com",
    "api.al-array.com",
    "apks.rp.al-array.com",
    "sensic.net",
    "clickblitzo.com",
    "pubmatic.com",
    "openx.net",
...
CustomRoute myRoutes[] = {
    {"dns.me", IPAddress(0, 0, 0, 0)},
};

const int numRoutes = sizeof(myRoutes) / sizeof(myRoutes[0]);
...
const String htmlStyle =
    ...
    "button,input[type=submit]{width:100%;padding:12px;font-size:1rem;"
    "background:#eee;color:#000;border:1px solid "
    "#ccc;border-radius:4px;margin:10px 0;cursor:pointer;transition:0.2s;}"
    ...
    "input,select{width:100%;padding:12px;margin:10px 0;border:1px solid "
    "#ccc;border-radius:4px;font-size:1rem;box-sizing:border-box;}"
    ".footer{font-size:0.85rem;color:#555;text-align:center;margin-top:40px;}"
    ".terminal{font-family:'Courier "
    "New',monospace;font-size:0.9rem;background:#000;color:#0f0;padding:20px;"
    "border-radius:4px;overflow-x:auto;max-height:70vh;overflow-y:auto;white-"
    "space:pre-wrap;word-wrap:break-word;line-height:1.4;}"
    ".modal{display:none;position:fixed;top:0;left:0;width:100%;height:100%;"
    "background:rgba(0,0,0,0.5);align-items:center;justify-content:center;}"
    ".modal-content{background:#fff;padding:30px;border-radius:8px;text-align:"
...
const String modalHTML =
    ...
    "<p id='modalMessage'></p>"
    "<button "
    "onclick=\"window.location.href=document.getElementById('confirmLink')."
    "value\">Yes</button>"
    "<button "
...
void handleToggleDoH() {
  ...
  String html = "<!DOCTYPE html><html><head><title>Switching Mode</title>" +
                htmlStyle +
                "</head>"
...
void handleConfig() {
  if (!checkAuth())
    return;
  String modeStatus = useDoH
                          ? "<span style='color:green'>ENCRYPTED (DoH)</span>"
                          : "<span style='color:blue'>STANDARD (Speed)</span>";
  String toggleBtn =
      useDoH ? "<a href='/toggledoh'><button class='btn-red'>Disable DoH "
               "(Switch to Standard)</button></a>"
             : "<a href='/toggledoh'><button class='btn-green'>Enable DoH "
               "(Encrypt Traffic)</button></a>";

  String html =
      "<!DOCTYPE html><html><head><title>ESP32 DNS Tool</title>" + htmlStyle +
      "</head><body>"
      "<div class='container'>"
      ...
      "<hr>" +
      toggleBtn +
      "<a href='/resetwifi' onclick=\"return confirmAction('Are you sure you "
      "want to change WiFi network? This will clear saved credentials.', "
      "'/resetwifi')\"><button>Change WiFi Network</button></a>"
      "<a href='/reboot' onclick=\"return confirmAction('Are you sure you want "
      "to reboot the device?', '/reboot')\"><button>Reboot Device</button></a>"
      "<a href='/log'><button>View Live Log</button></a>"
      "<p class='footer'>They think they own your traffic.<br><strong>They are "
      "wrong.</strong></p>" +
      modalHTML + "</div></body></html>";
...
void handleRoot() {
  ...
  for (int i = 0; i < numScanned; i++) {
    options += "<option value='" + scannedSSIDs[i] + "'>" + scannedSSIDs[i] +
               "</option>";
  }
  String html =
      "<!DOCTYPE html><html><head><title>Setup</title>" + htmlStyle +
      "</head>"
      "<body><div class='container'>"
      "<h1>ESP32 DNS Tool</h1>"
      ...
      "</form>"
      "<p class='footer'>They think they own your traffic.<br>They are "
      "wrong.</p>"
      "</div></body></html>";
  server.send(200, "text/html", html);
}
...
void handleLog() {
  if (!checkAuth())
    return;
  String logHtml = "<!DOCTYPE html><html><head><title>Live Log</title>" +
                   htmlStyle +
...
void updateLED() {
  ...
  if (protectionActive) {
    if (current - ledMillis >= 20) {
      ...
      if (ledBrightness <= 50 || ledBrightness >= 255)
  ...
  if (connectSuccess) {
    ...
    server.onNotFound([]() {
      server.sendHeader("Location",
                        "http://" + WiFi.localIP().toString() + "/config");
      server.send(302, "text/plain", "");
    });
...
void loop() {
  ...
  if (packetSize) {
    ...
    String clientIP = udp.remoteIP().toString();

    bool customFound = false;
    for (int i = 0; i < numRoutes; i++) {
      if (domain == String(myRoutes[i].domain)) {
        IPAddress target = myRoutes[i].targetIp;
        if (String(myRoutes[i].domain) == "dns.me")
          target = WiFi.localIP();
        addLog("CUSTOM (" + clientIP + "): " + domain + " -> " +
               target.toString());
        buildDNSResponse(packetBuffer, len, target);
        customFound = true;
        break;
      }
    }
    ...
    for (int i = 0; i < numBlocked; i++) {
      if (domain == String(blockedDomains[i]) ||
          domain.endsWith("." + String(blockedDomains[i]))) {
        isBlocked = true;
        break;
      }
    }

    addLog("Q (" + udp.remoteIP().toString() + "): " + domain +
    ...
    if (isBlocked) {
      ...
      buildDNSResponse(packetBuffer, len, IPAddress(0, 0, 0, 0));
    } else {
      if (useDoH) {
